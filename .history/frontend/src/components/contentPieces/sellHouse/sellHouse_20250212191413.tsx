...
const SellHouse = () => {
  const [currentColor, setCurrentColor] = useState(COLORS_GREEN[0]);
  ...
  useEffect(() => {
    setCurrentColor(COLORS_GREEN[colorIndex]);
  }, [colorIndex]);

  return (
    <section className="w-screen flex justify-center items-center h-screen">
      <motion.div className="cursor-pointer transition-colors duration-2000 ease-in-out">
        <svg
