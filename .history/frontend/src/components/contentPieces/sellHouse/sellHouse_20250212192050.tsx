...
// Array of greens to cycle through
const COLORS_GREEN = [
  "#00FF00", // Light green
  "#32CD32", // Lime green
  "#228B22", // Forest green
...
const SellHouse = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setColorIndex((prevIndex) => (prevIndex + 1) % COLORS_GREEN.length);
  ...
  return (
    <section className="w-screen flex justify-center items-center h-screen">
      <motion.div className="cursor-pointer" animate={controls}>
        <svg
        ...
        >
          ...
          <defs>
            <linearGradient id="greenGradient" x1="0%" y1="0%" x2="100%" y2="100%">
              {/* Motion element with stopColor */}
              <motion.stop
                offset="0%"
                style={{ stopColor: color.get() }} // Get the current color from the motion value
                transition={{ ease: "easeInOut", duration: 2 }}
              />
              <motion.stop
                offset="100%"
                style={{ stopColor: "#006400" }} // Static end color
                transition={{ ease: "easeInOut", duration: 2 }}
              />
            </linearGradient>
          </defs>
          ...
          <g>
            {/* Apply the animated color gradient */}
