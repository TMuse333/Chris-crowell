...
const Video: React.FC<VideoProps> = ({ src, alt, thumbnail, muted }) => {
  ...
  // Track if the video is in view
  const isInView = useInView(videoRef, {
    once: false,
    margin: "0px", // Adjust if you want buffer
    amount: 0.5, // Play when 50% of video is visible
  });

  // Handle play/pause for muted videos based on visibility
  useEffect(() => {
    if (videoRef.current && muted) {
      if (isInView) {
        ...
        console.warn('video should pause')
        videoRef.current.pause();
        // Optionally reset to start if desired
        videoRef.current.currentTime = 0;
      }
    }
  }, [isInView, muted]);

  // Handle click to play for non-muted videos with thumbnail
