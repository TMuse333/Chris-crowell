...
const Carousel: React.FC<CarouselProps> = ({ images }) => {
  ...
  const tabWidth = `calc(100% / ${images.length})`; // Calculate width for each tab based on number of images
  ...
  return (
    ...
    >
      ...
      {/* Container for both tabs and pause button */}
      <div className="absolute bottom-[5%] left-1/2 -translate-x-1/2 flex items-center">
        ...
        <div className="flex w-full justify-center">
          {images.map((_, index) => (
            <div
              key={index}
              className={`h-[8px] relative`}
              style={{ width: tabWidth }} // Apply dynamic tab width
              onClick={() => setCurrentIndex(index)}
            >
              <div
                className={`bg-gray-600 hover:scale-[1.2] transition-all cursor-pointer`}
              >
                {index === currentIndex && (
                  <div
        ...
        <button
          ...
          className="ml-4 text-white text-xl px-4 py-2 bg-gray-700 rounded-full hover:bg-gray-600 transition"
