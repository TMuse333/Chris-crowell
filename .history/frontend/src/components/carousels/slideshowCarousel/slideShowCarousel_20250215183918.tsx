...
const getCircleWidth = (index: number) => {
  ...
  -translate-y-1/2" onClick={resetSlideShow}>
    <FaRedo  />
  </div>
) : slideShowPaused ? (
  <div className="ml-auto mr-auto scale-[1.5]
  absolute top-1/2 left-1/2 -translate-x-1/2
  -translate-y-1/2" onClick={togglePlay}>
    <FaPlay  />
  </div>
) : (
  <div className="ml-auto mr-auto scale-[1.5]" onClick={togglePlay}>
    <FaPause  />
  </div>
)}
...
useEffect(() => {
  ...
  return (
      <section 
          id={`carousel-element-${index} text-black`}
