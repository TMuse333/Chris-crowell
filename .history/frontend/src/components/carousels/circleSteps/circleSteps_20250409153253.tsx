...
const CircleStep = ({ number, title, description, inView, index,
  ...
  <div className={ `relative  h-[100px] flex items-center justify-center mb-4 mx-auto
  ${boxTextColor ? `${boxTextColor}` : ''}`}>
    ...
    <div className={`absolute inset-0 flex items-center justify-center text-3xl font-bold ${boxTextColor ? `${boxTextColor}` : ''}`}>
  {number}
</div>
</div>

<h3 className={`text-xl sm:text-2xl md:text-3xl font-semibold mt-6 md:px-6 w-[40vw] text-center mx-auto ${boxTextColor ? `${boxTextColor}` : ''}`}>
  {title}
</h3>

<p className={`text-sm sm:text-md md:text-lg text-center mt-2 max-w-[350px] mx-auto ${boxTextColor ? `${boxTextColor}` : ''}`}>
  {description}
</p>
...
//   };
  ...
  return (
    ...
    `}>
     <h2 className={`text-4xl sm:text-6xl text-center font-bold pb-6 ${textColor ? `${textColor}` : ''}`}>
  {title}
</h2>

<p className={`px-4 text-center sm:text-lg md:text-xl ${textColor ? `${textColor}` : ''}`}>
  {description}
</p>
...
{steps.map((step, index) => {
  ...
  return (
    <motion.div
      key={index}
      className={`relative z-10 px-4 w-[80vw]
      ${boxTextColor ? `${boxTextColor}` : ''}
       ${boxColor} h-[70vh] rounded-2xl ${index === steps.length - 1 && bigLast ?  'w-screen mr-12' : ''}`}
    >
      <CircleStep 
      boxTextColor={boxTextColor}
      {...step} index={index} inView={inView} />
    </motion.div>
  );
})}
