...
const CircleStep = ({ number, title, description, inView, index,
  ...
  <h3 className={`text-xl sm:text-2xl md:text-3xl font-semibold mt-6 md:px-6  max-w-[550px] text-center mx-auto ${boxTextColor ? `${boxTextColor}` : ''}`}>
  {title}
</h3>

<p className={`text-sm sm:text-md md:text-lg text-left mt-2  mx-auto px-4 ${boxTextColor ? `${boxTextColor}` : ''}`}>
  {description}
</p>
...
//   };
  ...
  return (
    <section  key={1} className={`  ${bgColor ? `${bgColor}` : ""} 
...
<p className={`px-4 text-center sm:text-lg md:text-xl ${textColor ? `${textColor}` : ''}`}>
  {description}
      ...
      <div ref={containerRef} className="relative w-screen h-screen  overflow-hidden z-10">
        <div
          ref={trackRef}
          key={2}
...
{steps.map((step, index) => {
  ...
  return (
    <motion.div
      key={index}
      className={`relative z-10 w-[95vw] max-w-[600px]
