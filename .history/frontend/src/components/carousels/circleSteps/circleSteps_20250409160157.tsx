...
const CircleStep = ({ number, title, description, inView, index,
  ...
  <h3 className={`text-xl sm:text-2xl md:text-3xl font-semibold mt-6 md:px-6 max-w-[350px] text-center mx-auto ${boxTextColor ? `${boxTextColor}` : ''}`}>
  {title}
</h3>

<p className={`text-sm sm:text-md md:text-lg text-left mt-2 max-w-[350px] mx-auto px-4 ${boxTextColor ? `${boxTextColor}` : ''}`}>
  {description}
</p>
...
{steps.map((step, index) => {
  ...
  return (
    <motion.div
      key={index}
      className={`relative z-10 w-[95vw]
