...
const TypeAlongText: React.FC<TypeAlongTextProps> = ({
  ...
  return (
    <motion.span
        ...
        className={`inline-block whitespace-pre-line ${highlightComplete && isKeyword(word) ? 'animate-gradient' : ''}`}
        style={
            highlightComplete && isKeyword(word)
                ? {
                      ...
                      backgroundClip: 'text',
                      WebkitBackgroundClip: 'text', // Ensure it works on webkit browsers
                      color: 'transparent',
                      transition: 'background-image 3s ease-in-out infinite',
                  }
                : undefined
        }
        whileHover={{
