...
interface SubMenuProps {
  ...
  const bigLinks = [
   ...
   
    {
      ...
      listSubMenu: true,
      subMenuSrc:brain.src,
      subMenuAlt:'Two hands',
      desktopDescription:'Learn about various real estate services that I can assist you with'
    },
    {
      name: 'Contact',
  ...
  const DesktopSubMenu:React.FC<DesktopSubMenuProps> = ({
    ...
    return (
        <div className={`bg-[#2EC9FF] fixed top-[20px] w-screen
...
<ul className="flex mt-6">
  ...
  = ({subMenuClicked, setSubMenuClicked,links}) => {
    ...
    return (
        <nav className={`fixed top-0 right-0 bg-gradient-to-b from-[#0077b3] to-blue-300
        h-screen w-[75vw] max-w-[470px] z-10 transition-transform
      ${!subMenuClicked ? 'translate-x-[200%]' : 'translate-x-0'}`}>
...
/>
  ...
  <Link href={link.destination? link.destination : ''}>
    <p className="my-auto text-sm relative z-[4] hover:text-black
    sm:text-md">
      {link.name}
    </p>
  </Link>
